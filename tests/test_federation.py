...
# ── Inbound: Bottleneck Escalation ─────────────────────────────


class TestInboundBottleneckEscalation:
    """OP_BOTTLENECK_ESCALATION creates typed tasks for the fix pipeline."""

    def test_bottleneck_creates_task(self, tmp_path):
        """Bottleneck escalation creates a [BOTTLENECK_ESCALATION] task."""
        from steward.federation import OP_BOTTLENECK_ESCALATION
        from steward.services import SVC_TASK_MANAGER
        from vibe_core.di import ServiceRegistry
        from vibe_core.task_management.task_manager import TaskManager
        ...
        result = bridge.ingest(
            OP_BOTTLENECK_ESCALATION,
            {
                ...
                "target": "ruff_clean,tests_pass",
                "requested_action": "fix",
                "severity": "high",
                "details": "Brain health scope gate blocked code-fix mission",
            },
        )
        assert result is True
        tasks = task_mgr.list_tasks()
        assert len(tasks) == 1
        assert tasks[0].title == "[BOTTLENECK_ESCALATION] scope_gate_block: ruff_clean,tests_pass"
        assert tasks[0].priority == 80  # high severity

    def test_bottleneck_severity_to_priority(self, tmp_path):
        ...
        for severity, expected_priority in [("critical", 95), ("high", 80), ("medium", 60), ("low", 40)]:
            task_mgr = TaskManager(project_root=tmp_path / severity)
            ServiceRegistry.register(SVC_TASK_MANAGER, task_mgr)
            ...
            tasks = task_mgr.list_tasks()
            assert tasks[0].priority == expected_priority, f"severity={severity}"

    def test_bottleneck_missing_target_rejected(self, tmp_path):
        """No target → rejected."""
        from steward.federation import OP_BOTTLENECK_ESCALATION
        from steward.services import SVC_TASK_MANAGER
        from vibe_core.di import ServiceRegistry
        ...
        assert not bridge.ingest(
            OP_BOTTLENECK_ESCALATION,
            {"source_agent": "agent-city", "bottleneck_type": "test"},
        )

    def test_bottleneck_no_task_manager_returns_false(self):
        """Without TaskManager registered, bottleneck_escalation fails gracefully."""
        from steward.federation import OP_BOTTLENECK_ESCALATION

        bridge = FederationBridge()
        assert not bridge.ingest(
            OP_BOTTLENECK_ESCALATION,
            {"source_agent": "peer", "target": "fix_me"},
        )

    def test_bottleneck_trust_gated(self, tmp_path):
        """Untrusted peers cannot send bottleneck escalations."""
        from steward.federation import OP_BOTTLENECK_ESCALATION
        from steward.services import SVC_TASK_MANAGER
        from vibe_core.di import ServiceRegistry
        from vibe_core.task_management.task_manager import TaskManager
        ...
        reaper = HeartbeatReaper()
        reaper.record_heartbeat("untrusted-peer", source="test")
        # Set trust below floor
        peer = reaper.get_peer("untrusted-peer")
        peer.trust = 0.1

        task_mgr = TaskManager(project_root=tmp_path)
        ServiceRegistry.register(SVC_TASK_MANAGER, task_mgr)

        bridge = FederationBridge(reaper=reaper, delegation_trust_floor=0.3)
        result = bridge.ingest(
            OP_BOTTLENECK_ESCALATION,
            {"source_agent": "untrusted-peer", "target": "fix_me"},
        )
        assert result is False
        assert bridge._delegations_rejected == 1

    def test_bottleneck_stores_metadata_in_description(self, tmp_path):
        """Source, action, repo, and details stored in task description."""
        from steward.federation import OP_BOTTLENECK_ESCALATION
        from steward.services import SVC_TASK_MANAGER
        from vibe_core.di import ServiceRegistry
        from vibe_core.task_management.task_manager import TaskManager
        ...
        bridge.ingest(
            OP_BOTTLENECK_ESCALATION,
            {
                ...
                "details": "scope gate blocked",
            },
        )
        tasks = task_mgr.list_tasks()
        desc = tasks[0].description
        assert "source:agent-city" in desc
        assert "action:fix" in desc
        assert "repo:kimeisele/agent-city" in desc
        assert "details:scope gate blocked" in desc

    def test_bottleneck_via_transport(self, tmp_path):
        """Bottleneck escalation works end-to-end via transport ingest."""
        from steward.federation import OP_BOTTLENECK_ESCALATION
        from steward.services import SVC_TASK_MANAGER
        from vibe_core.di import ServiceRegistry
        from vibe_core.task_management.task_manager import TaskManager
        ...
        transport = FakeTransport(
            messages=[
                {
                    "operation": OP_BOTTLENECK_ESCALATION,
                    "payload": {
                        "source_agent": "agent-city",
                        "bottleneck_type": "scope_gate_block",
                        "target": "ruff_clean",
        ...
        bridge = FederationBridge()
        processed = bridge.process_inbound(transport)
        assert processed == 1
        tasks = task_mgr.list_tasks()
        assert len(tasks) == 1
        assert "[BOTTLENECK_ESCALATION]" in tasks[0].title
