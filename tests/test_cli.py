...
from steward.types import AgentEvent, AgentUsage, ToolUse
...
class TestCLIFormatting:
    ...
    def test_format_text_delta(self, capsys):
        _format_event(AgentEvent(type="text_delta", content="chunk"))
        captured = capsys.readouterr()
    ...
    def test_format_done_with_usage(self, capsys):
        ...
        _format_event(AgentEvent(type="done", usage=usage))
        captured = capsys.readouterr()
...
class TestJSONOutput:
    def test_json_text_event(self, capsys):
        _format_event_json(AgentEvent(type="text", content="hello"))
    ...
    def test_json_tool_call(self, capsys):
        ...
        _format_event_json(AgentEvent(type="tool_call", tool_use=tu))
        line = capsys.readouterr().out.strip()
        obj = json.loads(line)
    ...
    def test_json_done_event(self, capsys):
        ...
        _format_event_json(AgentEvent(type="done", usage=usage))
        line = capsys.readouterr().out.strip()
        obj = json.loads(line)
    ...
    def test_json_error_event(self, capsys):
        _format_event_json(AgentEvent(type="error", content="fail"))
        line = capsys.readouterr().out.strip()
        obj = json.loads(line)
