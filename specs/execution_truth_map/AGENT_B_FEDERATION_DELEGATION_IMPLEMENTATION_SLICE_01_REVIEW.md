...
The four blockers from the first implementation review are addressed exclusively by
the following new commits; no ADR, contract or implementation-plan scope was expanded:

| Repository | Revision commit | Remote branch |
|---|---|---|
| `kimeisele/steward` | `0248dc4efbd514eaf20bb2c1ed79101d87eb4516` | `impl/federation-delegation-slice-01` |
| `kimeisele/agent-city` | `0caa6fb536cc5b5883ffde89e66282af5c6e26bc` | `impl/federation-delegation-slice-01` |

### 11.1 Origin request conflict closure

`FederationV1Origin.create` now first reads the durable record for the requested
`delegation_id`. If the request is already present, it computes the semantic digest and
compares the complete original request identity: source/target, message and request-root
IDs, issued/expiry values, signer public key, key ID and the complete payload including
the derived digest/idempotency key. An exact match returns the stored immutable request
wire and stored carrier without rebuilding or sending bytes. Any changed digest,
message ID, display/payload field, time or wire metadata records
`origin_request_conflict` in the origin ledger and raises the same stable local code.

For a new request, `create_request` returns the record produced by the atomic commit;
`create` compares the generated bytes/carrier with those stored bytes and returns only
the stored values. A race between two origin instances therefore cannot return an
unpersisted carrier. The origin ledger keeps the conflict finding in its durable
`findings` array.

### 11.2 Receipt causal and target binding

Before any origin-ledger mutation, `OriginDelegationLedger.apply_receipt` now requires
all of these exact equalities:

```text
receipt.source_node_id       == origin_record.target_node_id
receipt.target_node_id       == origin_record.origin_node_id
receipt.payload.delegation_id == origin_record.delegation_id
receipt.correlation_id       == origin_record.delegation_id
receipt.request_message_id   == origin_record.request_message_id
receipt.causation_message_id == origin_record.request_message_id
```

A validly signed receipt from another registered node, a wrong correlation or
causation ID, or a record whose original target no longer matches the receipt produces
the stable `receipt_correlation_conflict`; the origin record is not applied. The local
finding is persisted before the rejection. The existing request-root, receipt-ID,
receipt-content-digest, receipt-wire-byte and first-set `target_work_id` checks remain
in force after these binding checks.

### 11.3 Typed provenance boundary

Both repo-local adapters now require an instance of
`ValidatedFederationV1KeyRegistry`; a plain `dict` or untyped mapping is rejected as
`registry_unvalidated`. The only construction path is
`ValidatedFederationV1KeyRegistry.from_provenance(...)`, guarded by a private creation
token. It validates, for each supplied Root Enrollment and Signing-Key Certificate:

- closed enrollment/certificate field sets and exact version literals;
- standard Base64 lengths for root, signer and signatures;
- root `node_id = ag_ + SHA256(ASCII(hex(root_public_key)))[:32]`;
- certificate `key_id = key_ + SHA256(signing_public_key)`;
- root-key equality between enrollment and certificate;
- root-signed domain-separated enrollment and certificate body digests;
- registry, certificate and activation epochs, including registry-epoch binding;
- activation-at equals not-before and a strictly later not-after;
- exact RFC-3339 UTC validity at the snapshot time;
- `rotation_kind` in the frozen V1 set and a revocation reference state;
- duplicate node/key provenance conflicts.

`lookup(key_id, at=message.issued_at)` rejects revoked or out-of-window signing keys.
Thus the envelope validator receives only a provenance-validated immutable snapshot and
does not treat arbitrary registry data as authority. Tests cover wrong key ID, wrong
node, invalid root certificate signature, not-yet-active certificate, expired
certificate, revoked key at message validation time and unvalidated/corrupt registry
state.

### 11.4 Ledger corruption and inter-process safety

Both ledgers now distinguish a missing file from a corrupt one. Missing files resolve to
an empty in-memory ledger. Existing malformed JSON, invalid top-level shape, unknown
top-level fields, missing `findings`, invalid finding records, missing record fields,
wrong delegation key, invalid state or invalid send state raise `ledger_corrupt`; they
are never reset to an empty ledger.

Every ledger `get`, `commit`, `create_request`, `apply_receipt`, `mark_*_sent` and
finding write holds both the existing thread lock and a repo-local same-directory
`fcntl.flock` lock file around the complete read-modify-write. `os.replace` remains the
atomic file exchange inside that critical section. Consequently independent ledger
instances cannot lose a concurrent identical admission, conflicting admission,
parallel updates for different delegations or a newer send-status update.

### 11.5 Revision test evidence

After the revision commits, the following exact commands passed (each emitted only the
same unrelated existing `steward-protocol` deprecation warning):
...
| Steward cross-repo: `AGENT_CITY_REPO=/Users/ss/projects/agent-city pytest -q tests/test_federation_v1_cross_repo_crucible.py` | **1 passed** |
| Steward legacy: `pytest -q tests/test_federation_gateway.py tests/test_federation_quarantine.py` | **61 passed** |
| Agent City legacy Federation: `pytest -q tests/test_federation_nadi.py tests/test_federation_relay.py` | **90 passed** |
| Steward new adapter/tests Ruff + `py_compile` | **clean / pass** |
| Agent City new adapter/tests Ruff + `py_compile` | **clean / pass** |

The new tests explicitly exercise: identical second `create`, changed payload and
message-ID conflicts, durable origin findings, wrong registered receipt source,
correlation and causation mutations, wrong original target, all provenance/time/
revocation cases, corrupt JSON and invalid ledger shape, two independent ledger
instances racing on one admission, two independent admissions racing on one file, and
parallel `mark_*_sent` updates that must not roll back each other.

The feature gate remains default-disabled and the cross-repo crucible remains
admission-only. Worker/tool execution, Status Query, recovery automation,
verification, Managed-Task completion, Provider Failover, Context Bridge, Execution
Spine and production activation remain unchanged and excluded.

## 12. Revised Agent-B decision request

Please re-check only the four former blockers against the two revision SHAs:

1. Can any `FederationV1Origin.create` return bytes or a carrier that are not the
   immutable origin-ledger bytes, including under a same-delegation race?
2. Can a validly signed but wrong-node or wrong-causation receipt mutate the origin
   ledger?
3. Can an untyped, root-unauthorized, inactive, expired or revoked registry record
   reach envelope signature acceptance?
4. Can corruption or concurrent writers reset, overwrite or roll back a ledger state?

Requested decision: `ACCEPTED FOR MERGE REVIEW` or a precise remaining code/test
blocker. No wider feature or architecture review is requested.
