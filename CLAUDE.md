...
Git: main, 2 dirty, ci=success, 0 open PRs
Project: python
...
| `SVC_IMMUNE` | StewardImmune — unified self-healing system.
...
| `SVC_NORTH_STAR` | North Star — infrastructure-level goal seed (not LLM prompt).
...
| 2 | BUDDHI | antahkarana | Discriminate: tool selection, verdicts, token budg |
...
| 10 | SHROTRA | jnanendriya | Hear project history via git (branch, commits, ups |
| 11 | TVAK | jnanendriya | Feel project structure via filesystem (language, d |
| 12 | CHAKSHUS | jnanendriya | See code structure via AST (modules, classes, func |
| 13 | RASANA | jnanendriya | Taste code quality via test framework (results, co |
| 14 | GHRANA | jnanendriya | Smell code entropy via file metrics (staleness, si |
...
| 25 | JIVA | para_prakriti | The living entity — infinitely potent but tiny (1/ |